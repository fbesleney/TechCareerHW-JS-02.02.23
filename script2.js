...
let ordersOf96 = orders.filter(x => x.orderDate.startsWith('1996'));

    console.log('1. 1996 yılında kaç adet sipariş geçilmiştir? : ', ordersOf96);



// 2. Bir seferde yapılan en yüksek satış nedir?

let maxPrice = orders.sort(function(a,b){
    return a.unitPrice - b.unitPrice; 
});

    console.log('2. Bir seferde yapılan en yüksek satış nedir? : ', maxPrice.slice(-1));



// 3. USA de toplam satış cirom nedir?

let orderOfUSA = orders.filter(q => q.shipAddress.country == "USA");  // Tüm USA Siparişlerini filtrelemek için

console.log('3. Tüm USA siparişleri : ',  orderOfUSA);

function getUnitPrice(x, y) {                                       
    let allPrices = [];
    for (var i=0; i < x.length ; i++)
      allPrices.push(x[i]["details"][0][y]);
    return allPrices;
  }
  
  let result = getUnitPrice(orderOfUSA, "unitPrice");                   // USA'de yapılan siparişlerin unitPrice'larını ayıklamak için
  console.log('3. USA de yapılan satışların unit Price\'lar listesi : ', result);
